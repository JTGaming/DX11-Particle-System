...
struct ImVec4
{
    ...
    ImVec4 Scaled(float scale) { return ImVec4(x * scale, y * scale, z * scale, w); }
    ImVec4 AlphaScaled(const float& scale) { return ImVec4(x, y, z, w * scale); }
...
namespace ImGui
{
    ...
    IMGUI_API void          SetCursorPosYScale(float local_y);                                   //
    ...
    IMGUI_API bool          SelectableButton(char const* label, bool selectable, int* p_selected, int v_button, const ImVec2& size_arg);
    IMGUI_API void          CenteredText(const char* text);
    ...
    IMGUI_API bool          SliderInt(const char* label, unsigned long long* v, unsigned long long v_min, unsigned long long v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    ...
    IMGUI_API bool			MyColorEdit4(char const* label, MenuColor* color_multi, ImGuiColorEditFlags flags = 0x01); // 0x01 = ImGuiColorEditFlags_Alpha = very dodgily backward compatible with 'bool show_alpha=true'
    IMGUI_API bool			MyColorEdit3(char const* label, MenuColor* color_multi, ImGuiColorEditFlags flags = 0x01); // 0x01 = ImGuiColorEditFlags_Alpha = very dodgily backward compatible with 'bool show_alpha=true'
...
enum ImGuiCol_
{
    ...
    ImGuiCol_Button,
    ImGuiCol_ButtonInactive,
    ImGuiCol_ButtonHovered,
    ImGuiCol_ButtonActive,
    ImGuiCol_ButtonHoveredActive,
    ImGuiCol_Header,                // Header* colors are used for CollapsingHeader, TreeNode, Selectable, MenuItem
    ImGuiCol_HeaderInactive,
    ImGuiCol_HeaderHovered,
    ImGuiCol_HeaderActive,
    ImGuiCol_HeaderHoveredActive,
...
struct ImGuiIO
{
    ...
    float       CurrentTime;                      // = 1.0f/60.0f     // Time elapsed since last frame, in seconds.
...
struct ImColor
{
    ...
    inline ImColor ShiftHSV(float h, float s, float v, float a = 1.0f) { float r, g, b, hg, sg, vg; ImGui::ColorConvertRGBtoHSV(Value.x, Value.y, Value.z, hg, sg, vg); ImGui::ColorConvertHSVtoRGB(h + hg, s + sg, v + vg, r, g, b); return ImColor(r, g, b, a); }
};
...
namespace ImGui
{
    ...
    static inline bool  SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, float multiple, bool enabled_scaling = false)
    {
        if (enabled_scaling)
        {
            *v *= multiple;
            v_min *= multiple;
            v_max *= multiple;
            bool retval = SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format);
            *v /= multiple;
            return retval;
        }
        else
            return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, multiple);
    }
