...
bool ImGui::SelectableButton(char const* label, bool selectable, int* p_selected, int v_button, const ImVec2& size_arg)
{
    if (ButtonEx(label, *p_selected == v_button && selectable, size_arg, 0)) {
        *p_selected = v_button;
        return true;
    }
    return false;
...
void ImGui::CenteredText(const char* text)
{
    ...
    SetCursorPosX((window->Size.x - ImGui::CalcTextSize(text).x) * 0.5f);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHoveredActive : ImGuiCol_Button);
...
bool ImGui::ButtonEx(char const* label, bool selected, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    const ImU32 col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : (hovered && selected) ? ImGuiCol_ButtonHoveredActive : hovered ? ImGuiCol_ButtonHovered : selected ? ImGuiCol_Button : ImGuiCol_ButtonInactive);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_SliderGrabActive), style.GrabRounding);
...
bool ImGui::SliderInt(const char* label, unsigned long long* v, unsigned long long v_min, unsigned long long v_max, const char* format, ImGuiSliderFlags flags)
{
...
IMGUI_API bool ImGui::MyColorEdit4(char const* label, MenuColor* color, ImGuiColorEditFlags flags) {
    float* colr = color->GetColor();
    return ColorEdit4(label, colr, flags);
}

IMGUI_API bool ImGui::MyColorEdit3(char const* label, MenuColor* color, ImGuiColorEditFlags flags) {
    float* colr = color->GetColor();
    return ColorEdit3(label, colr, flags);
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    //if (hovered || selected)
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : (selected && hovered) ? ImGuiCol_HeaderHoveredActive : hovered ? ImGuiCol_HeaderHovered : selected ? ImGuiCol_Header : ImGuiCol_HeaderInactive);
